...
export class Garage extends Component {
  ...
  private async generateRandomCars(): Promise<void> {
    this.garageOptions.optionsButtons.btnRace.setDisabled(true);
    this.garageOptions.optionsButtons.btnGenerateCars.setDisabled(true);
    ...
    await this.getAllCars(this.page);
    this.garageOptions.optionsButtons.btnRace.setDisabled(false);
    this.garageOptions.optionsButtons.btnGenerateCars.setDisabled(false);
  }
  ...
  private async startRaceAllCars(): Promise<void> {
    ...
    this.garageOptions.optionsButtons.btnReset.removeDisabled();
    setTimeout(() => this.winnerPopup?.destroy(), 3000);
  ...
  private resetAllCars(): void {
    this.garageOptions.optionsButtons.btnReset.setDisabled(true);
    ...
    Promise.all(allCarsToReset).then(() => {
      this.garageOptions.optionsButtons.btnRace.removeDisabled();
      this.garageOptions.optionsButtons.btnGenerateCars.removeDisabled();
    });
  }
}
