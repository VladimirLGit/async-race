...
export class GarageItem extends Component {
  private startEngineBtn: WEBUIButton;
  private stopEngineBtn: WEBUIButton;
  ...
  constructor(parentNode: HTMLElement, car: ICar) {
    ...
    const selectBtn = new WEBUIButton(carTop.element, ['btn-small'], 'select');
    selectBtn.onClickButton = () => {
      if (car.id) this.updateCar(car.id);
    };
    const removeBtn = new WEBUIButton(carTop.element, ['btn-small'], 'remove');
    ...
    this.startEngineBtn = new WEBUIButton(
      controlBlock.element,
      ['btn-small'],
      'A',
    );
    this.startEngineBtn.onClickButton = () => {
      if (car.id) this.startCarEngine(car.id);
    };
    this.stopEngineBtn = new WEBUIButton(
      controlBlock.element,
      ['btn-small'],
      'B',
      true,
    );
    this.stopEngineBtn.onClickButton = () => {
  ...
  updateBtns(type = false): void {
    this.startEngineBtn.setDisabled(type);
    this.stopEngineBtn.setDisabled(!type);
  }
  ...
  async startCarEngine(carId: number): Promise<void> {
    ...
    if (data.status === 200) {
      ...
      const { result } = data;
      this.animationCar(result.distance / result.velocity);
  ...
  disableAllButtons(): void {
    this.startEngineBtn.setDisabled(true);
    this.stopEngineBtn.setDisabled(false);
  }
}
